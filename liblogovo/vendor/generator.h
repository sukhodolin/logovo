...
struct _fn {
  ...
  auto operator()(_Range&& __r) const
      noexcept(noexcept(__r.begin())) -> decltype(__r.begin()) {
    return __r.begin();
  }
  ...
  auto operator()(_Range&& __r) const
      noexcept(noexcept(begin(__r))) -> decltype(begin(__r)) {
    return begin(__r);
  }
};
...
struct _fn {
  ...
  auto operator()(_Range&& __r) const
      noexcept(noexcept(__r.end())) -> decltype(__r.end()) {
    return __r.end();
  }
  ...
  auto operator()(_Range&& __r) const
      noexcept(noexcept(end(__r))) -> decltype(end(__r)) {
    return end(__r);
  }
};
}  // namespace __end
...
class __promise_base_alloc {
  static constexpr std::size_t __offset_of_allocator(
      std::size_t __frameSize) noexcept {
    return __aligned_allocation_size(__frameSize, alignof(_Alloc));
  }

  static constexpr std::size_t __padded_frame_size(
      std::size_t __frameSize) noexcept {
    return __offset_of_allocator(__frameSize) + sizeof(_Alloc);
  }

  static _Alloc& __get_allocator(
      void* __frame, std::size_t __frameSize) noexcept {
 ...
 public:
  ...
  static void operator delete(void* __ptr, std::size_t __frameSize) noexcept {
    ...
    __alloc.~Alloc();
    __localAlloc.deallocate(
        static_cast<std::byte*>(__ptr), __padded_frame_size(__frameSize));
  }
};
...
struct __generator_promise_base {
  ...
  struct __final_awaiter {
    ...
    std::coroutine_handle<> await_suspend(
        std::coroutine_handle<_Promise> __h) noexcept {
  ...
  std::suspend_always yield_value(_Ref&& __x) noexcept(
      std::is_nothrow_move_constructible_v<_Ref>) {
  ...
  std::suspend_always yield_value(_T&& __x) noexcept(
      std::is_nothrow_constructible_v<_Ref, _T>) {
  ...
  __yield_sequence_awaiter<generator<_Ref, _OValue, _OAlloc>> yield_value(
      std::ranges::elements_of<generator<_Ref, _OValue, _OAlloc>>
          __g) noexcept {
 ...
 public:
  ...
  class iterator {
   ...
   private:
    ...
    explicit iterator(
        __promise_base* __promise, std::coroutine_handle<> __coro) noexcept
